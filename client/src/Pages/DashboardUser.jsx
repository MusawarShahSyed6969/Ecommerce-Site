...
const DashboardUser = () => {
  ...
  {/* Main layout */}
  <div className="flex flex-col md:flex-row gap-4 w-full min-h-screen" style={{ padding: 24 }}>
    ...
    >
      <div className="md:min-h-screen flex flex-col gap-6 justify-start items-center">
        {Tabs.map((item) => (
          <div key={item.name} className="h-full flex min-w-full gap-6 justify-center items-center">
            <button
    ...
    <div className="md:hidden">
      {!ShowMenu && <IoMdArrowDropdownCircle onClick={() => setShowMenu(true)} className="w-8 h-8" />}
    </div>

    {/* Main content */}
    <div className="flex-1 bg-bg-secondary w-full min-h-screen flex flex-col">
      {CurrentTab === "Profile" && <ProfileSetting />}
      {CurrentTab === "Security" && <ProfileSecurity />}
      {CurrentTab === "Orders" && <DashboardOrders />}
      {CurrentTab === "Support" && <DashboardSupport />}
    </div>
  </div>
